...
| `writer.js` | "Generate with AI" for the other free-text boxes (`POST /api/ai/write`): review replies (farmer), reviews and report details (customer), decline reasons (farmer), FAQ answers, market descriptions, announcements and moderation notes (admin). Each kind says who may ask for it; Claude writes it with a key, otherwise a built-in writer fills it from the page's details (English or Urdu). 30 requests a minute per visitor |
