...
export default function Unsubscribe() {
  ...
  const [state, setState] = useState({ status: token ? 'working' : 'error', message: token ? '' : 'This unsubscribe link is not complete.' });
      ...
      <div className="panel text-center mx-auto" style={{ maxWidth: 520 }}>
        {state.status === 'working' && (
          <>
            ...
            <p className="mb-0">Unsubscribing…</p>
          </>
        )}
        {state.status === 'done' && (
          <>
            <i className="bi bi-envelope-check display-5 text-success" aria-hidden="true" />
            <h1 className="h3 mt-3">You are unsubscribed</h1>
            <p className="text-muted-2">
              {state.message}
              {state.email && <span className="d-block small mt-1">{state.email}</span>}
            </p>
            <p className="small mb-4">Changed your mind? You can sign up again at the bottom of any page.</p>
        ...
        {state.status === 'error' && (
          <>
            ...
            <h1 className="h3 mt-3">Could not unsubscribe</h1>
