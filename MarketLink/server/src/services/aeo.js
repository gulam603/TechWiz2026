...
export const HOW_IT_WORKS = [
  [
    "Find fresh produce",
    "Browse this week’s stock by category, market, farmer or pickup day.",
  ],
  [
    "Add to your basket",
    "Press Add on a product, choose how many and add it to the basket. One basket can hold several farmers.",
  ],
  [
    "Pick a pickup slot",
    "At checkout choose the market, day and time window for each farmer. No account is needed: guests get one automatically.",
  ],
  [
    "Collect and pay at the stall",
    "Show your order number at the stall on market day and pay the farmer in cash.",
  ],
];
...
const firstSentence = (s) =>
  (
    String(s || "")
      .split("\n")[0]
      .match(/^.*?[.!?](\s|$)/)?.[0] || String(s || "").split("\n")[0]
  ).trim();
const firstParagraph = (s) =>
  String(s || "")
    .split(/\n{2,}/)[0]
    .replace(/\s+/g, " ")
    .trim(); // the short, direct answer
...
const page = (title, lead, ...sections) =>
  `<header><h1>${esc(title)}</h1>${lead ? `<p>${esc(lead)}</p>` : ""}</header><main>${sections.filter(Boolean).join("")}</main>${SITE_NAV}`;
const section = (heading, html) =>
  html ? `<section><h2>${esc(heading)}</h2>${html}</section>` : "";

const productLine = (p) =>
  `${a(`/products/${p.slug}`, p.name)}: ${money(p.price)} per ${esc(p.unit)}${p.farmer?.stallName ? ` from ${esc(p.farmer.stallName)}` : ""}${p.status !== "available" || p.quantityAvailable <= 0 ? " (sold out this week)" : ""}`;
const marketLine = (m) =>
  `${a(`/markets/${m.slug}`, m.name)}${m.city ? `, ${esc(m.city)}` : ""}${m.operatingDays?.length ? `: open ${esc(days(m.operatingDays))}, ${esc(time12(m.openTime))} to ${esc(time12(m.closeTime))}` : ""}`;
const howItWorks = () =>
  `<ol>${HOW_IT_WORKS.map(([t, d]) => `<li><strong>${esc(t)}.</strong> ${esc(d)}</li>`).join("")}</ol>`;
const faqHtml = (faqs) =>
  faqs
    .map((f) => `<h3>${esc(f.question)}</h3>${paragraphs(f.answer)}`)
    .join("");
...
export const pageBody = {
  home: ({ faqs, categories, markets }) =>
    page(
      ...
      section(
        "Shop by category",
        list(categories.map((c) => a(`/products?category=${c.slug}`, c.name))),
      ),
  ...
  faq: ({ faqs }) =>
    page(
      ...
      ...Object.entries(FAQ_GROUPS).map(([key, label]) =>
        section(label, faqHtml(faqs.filter((f) => f.group === key))),
      ),
    ),

  product: ({ product: p, markets, reviews }) => {
    const soldOut = p.status !== "available" || p.quantityAvailable <= 0;
    const windows = (p.farmer?.pickupWindows || []).map(
      (w) =>
        `${DAY[w.day]} ${time12(w.start)} to ${time12(w.end)}${markets.find((m) => String(m._id) === String(w.market)) ? ` at ${markets.find((m) => String(m._id) === String(w.market)).name}` : ""}`,
    );
    return page(
      p.name,
      ...
      p.description
        ? section("About this product", paragraphs(p.description))
        : "",
      section(
        "Quick facts",
        list(
          [
            ...
            p.ratingCount > 0
              ? `Rating: ${p.ratingAvg} out of 5 from ${p.ratingCount} reviews`
              : "",
      ...
      section(
        "Markets",
        list(markets.map((m) => a(`/markets/${m.slug}`, m.name))),
      ),
      section(
        "Customer reviews",
        list(reviews.map((r) => `${r.rating}/5: ${esc(r.comment)}`)),
      ),
    );
  },
  ...
  farmer: ({ farmer: f, products, markets }) =>
    page(
      ...
      f.ratingCount > 0
        ? section(
            "Rating",
            `<p>${f.ratingAvg} out of 5 from ${f.ratingCount} reviews.</p>`,
          )
        : "",
    ),
  ...
  market: ({ market: m, farmers }) =>
    page(
      ...
      m.description
        ? section("About the market", paragraphs(m.description))
        : "",
      section(
        ...
        list(
          farmers.map(
            (f) =>
              `${a(`/farmers/${f.slug}`, f.stallName)}${f.city ? `, ${esc(f.city)}` : ""}`,
          ),
        ),
      ),
    ),
  ...
  products: ({ products, categories, category, title, description }) =>
    page(
      ...
      description,
      section(
        category ? `${category.name} this week` : "Popular this week",
        list(products.map(productLine)),
      ),
      section(
        "Categories",
        list(categories.map((c) => a(`/products?category=${c.slug}`, c.name))),
  ...
  markets: ({ markets, title, description }) =>
    page(
      title || "Farmers markets",
      description,
      section(
        "All markets",
        list(markets.map((m) => `${marketLine(m)}. ${esc(m.address)}`)),
      ),
    ),
    ...
    page(
      ...
      section(
        ...
        list(
          farmers.map(
            (f) =>
              `${a(`/farmers/${f.slug}`, f.stallName)}${f.city ? ` (${esc(f.city)})` : ""}${f.bio ? `: ${esc(firstSentence(f.bio))}` : ""}`,
          ),
        ),
      ),
    ),
  ...
  info: ({ pathname, title, description }) => {
    const extra = {
      "/about":
        ...
        section(
          "Who built it",
          "<p>MarketLink was built by Team Omniverse for the TechWiz 2026 eGreen Basket challenge.</p>",
        ),
      "/contact": section(
        "Contact details",
        list([
          "E-mail: hello@marketlink.pk",
          "Phone: +92 21 3456 7890",
          "Address: Aptech Learning Centre, F.B. Area, Karachi, Pakistan",
        ]),
      ),
      "/register/farmer": section(
        "Why sell on MarketLink",
        list([
          "Free to join",
          "Customers pre-order before market day, so you harvest the right amount",
          "Customers pay you in cash at your stall",
          "Orders, stock, pickup times and sales reports in one dashboard",
        ]),
      ),
    };
    return page(
...
async function siteData() {
  const [faqs, categories, markets, farmers] = await Promise.all([
    ...
    Market.find({ isActive: true })
      .select(
        "name slug city address operatingDays openTime closeTime description",
      )
      .sort({ city: 1, name: 1 })
      .lean(),
    Farmer.find({ isActive: true })
      .select("stallName slug city bio tags")
      .sort({ stallName: 1 })
      .lean(),
  ]);
...
const mdMarket = (origin, m) =>
  `- [${m.name}](${origin}/markets/${m.slug}): ${m.city ? `${m.city}, ` : ""}${m.address}. Open ${days(m.operatingDays)}, ${time12(m.openTime)} to ${time12(m.closeTime)}.`;
...
/** llms.txt: a short Markdown guide to the site for AI assistants (llmstxt.org). */
export async function llmsShortText(origin) {
  ...
  return [
    ...
    ...categories.map(
      (c) =>
        `- [${c.name}](${origin}/products?category=${c.slug})${c.description ? `: ${c.description}` : ""}`,
    ),
    "",
    "## Markets",
    ...
    "## FAQs",
    ...faqs.map(
      (f) => `- [${f.question}](${origin}/faq): ${firstParagraph(f.answer)}`,
    ),
    "",
    "## Optional",
...
/** llms-full.txt: the same guide with every answer, farmer and product (prices of this week). */
export async function llmsFullText(origin) {
  const [{ faqs, categories, markets, farmers }, products] = await Promise.all([
    siteData(),
    Product.find(Product.publicFilter())
      .populate("farmer", "stallName")
      .populate("category", "name")
      .select(
        "name slug price unit status quantityAvailable farmer category description",
      )
      .sort({ name: 1 })
      .lean(),
  ]);
  const byCategory = categories
    .map((c) => ({
      c,
      items: products.filter((p) => String(p.category?._id) === String(c._id)),
    }))
  ...
  return [
    ...
    ...farmers.map(
      (f) =>
        `- [${f.stallName}](${origin}/farmers/${f.slug})${f.city ? ` (${f.city})` : ""}${f.tags?.length ? ` [${f.tags.join(", ")}]` : ""}: ${firstSentence(f.bio)}`,
    ),
    "",
    ...
    ...byCategory.flatMap(({ c, items }) => [
      ...
      ...items.map(
        (p) =>
          `- [${p.name}](${origin}/products/${p.slug}): Rs ${p.price} per ${p.unit}, from ${p.farmer?.stallName}${p.status !== "available" || p.quantityAvailable <= 0 ? " (sold out)" : ""}`,
      ),
    ]),
    "",
  ].join("\n");
...
/** GET /llms.txt (cached for 10 minutes) */
export async function llmsTxt(req, res, origin) {
  const text = await cached(`short:${origin}`, () => llmsShortText(origin));
  res
...
/** GET /llms-full.txt (cached for 10 minutes) */
export async function llmsFullTxt(req, res, origin) {
  const text = await cached(`full:${origin}`, () => llmsFullText(origin));
  res
    .type("text/markdown; charset=utf-8")
    .set("Cache-Control", "public, max-age=600")
    .send(text);
